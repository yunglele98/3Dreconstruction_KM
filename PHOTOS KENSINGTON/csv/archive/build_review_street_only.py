...
for r in all_rows:
    if r[2] != 'confirmed':
        continue
    addr = r[1]
    has_number = bool(re.match(r'^\d', addr) or re.search(r'[~#]\d', addr) or re.match(r'^43\.6', addr))
    if has_number:
        continue
    label = r[1]
...
<head>
<meta charset="utf-8">
<title>Kensington Review: Street-Only Labels</title>
<style>
...
<body>
<div class="header">
  <h1>Kensington Review: Street-Only Labels (836 photos)</h1>
  <div class="stats">
...
for label, files in sorted_groups:
    ...
    parts.append(f'<div class="group" id="g_{gid}" data-label="{safe}">\n')
...
function exp(){
  ...
  a.download='photo_review_street_only_corrections.csv';
  a.click();
}
</script>
</body></html>''')

with open(BASE + 'review_street_only.html', 'w', encoding='utf-8') as f:
    f.write(''.join(parts))

print(f"Created review_street_only.html")
