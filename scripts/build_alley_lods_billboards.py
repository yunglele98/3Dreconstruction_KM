#!/usr/bin/env python3
"""Create LOD1/LOD2/BILLBOARD for HERO alley assets."""
...
def main():
    ...
    p.add_argument("--masters-dir", default="outputs/alleys/masters")
    p.add_argument("--lod-dir", default="outputs/alleys/lods")
    ...
    manifest = lod_dir.parent / "unreal_alley_lod_manifest.csv"
    with manifest.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(manifest_rows[0].keys()) if manifest_rows else ["alley_key", "lod_level", "fbx_path", "triangle_ratio_target"])
        w.writeheader()
        if manifest_rows:
