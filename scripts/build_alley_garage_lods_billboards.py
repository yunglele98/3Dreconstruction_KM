#!/usr/bin/env python3
"""Create LOD1/LOD2/BILLBOARD for HERO alley+garage assets."""
...
def main():
    ...
    p.add_argument("--masters-dir", default="outputs/alley_garages/masters")
    p.add_argument("--lod-dir", default="outputs/alley_garages/lods")
    ...
    for key in hero:
        ...
        if base is None:
            continue
        l1 = make_lod(base, 0.55, f"SM_{key}_LOD1")
        l2 = make_lod(base, 0.22, f"SM_{key}_LOD2")
        ...
        for o, suf in ((l1, "LOD1"), (l2, "LOD2"), (bb, "BILLBOARD")):
            ...
            out = lod_dir / f"SM_{key}_{suf}.fbx"
            export_selected(out)
            made += 1
            rows.append({
                ...
                "triangle_ratio_target": 0.55 if suf == "LOD1" else (0.22 if suf == "LOD2" else 0.0),
            })
            print(f"[OK] exported {out}")

    manifest = lod_dir.parent / "unreal_alley_garage_lod_manifest.csv"
    with manifest.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()) if rows else ["alley_garage_key", "lod_level", "fbx_path", "triangle_ratio_target"])
        w.writeheader()
        if rows:
            w.writerows(rows)
