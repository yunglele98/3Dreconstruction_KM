#!/usr/bin/env python3
"""Create HERO variants for top alley+garage types."""
...
def main():
    ...
    hero_keys = [s.strip() for s in Path(args.hero_file).read_text(encoding="utf-8").splitlines() if s.strip()]
    made = 0
    for key in hero_keys:
        ...
        if obj is None:
            continue
        obj.scale = (obj.scale[0] * 1.1, obj.scale[1] * 1.1, obj.scale[2] * 1.08)
