...
for f in sorted(PARAMS.glob("*.json")):
    ...
    if is_commercial:
        # Commercial: estimate width from building name
        # Names with multiple addresses (e.g. "374_College...362") suggest wide blocks
        name_len = len(f.stem)
        if name_len > 80:
        ...
        # Residential: use typical Kensington dims
