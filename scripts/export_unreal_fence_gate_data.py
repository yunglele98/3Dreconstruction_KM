...
OUT=ROOT/'outputs'/'fence_gates'; REF=OUT/'fence_gate_photo_references.csv'
...
def main()->int:
 OUT.mkdir(parents=True,exist_ok=True); conn=get_connection()
 try:
  ...
  for i,r in enumerate(refs,start=1):
   if r.get('has_coords')=='yes' and r.get('lat') and r.get('lon'): x,y=project(cur,float(r['lon']),float(r['lat']))
    ...
    a=anchors[(i-1)%len(anchors)]; x=float(a['x'])+((i%7)-3)*1.1; y=float(a['y'])+(((i//7)%7)-3)*1.1
   k=r['category']
   rows.append({'instance_id':f'fg_{i:04d}','source_table':'photo_ref','source_id':r.get('filename') or str(i),'fence_gate_key':k,'asset_path':f'/Game/Street/FenceGates/SM_{k}_A_standard','x_cm':f'{(x-ORIGIN_X)*100:.1f}','y_cm':f'{(y-ORIGIN_Y)*100:.1f}','z_cm':'0.0','yaw_deg':'0.0','uniform_scale':'1.000','metadata_json':json.dumps({'address_or_location':r.get('address_or_location','')},ensure_ascii=False)})
 finally:
  conn.close()
 out=OUT/'fence_gate_instances_unreal_cm.csv'
