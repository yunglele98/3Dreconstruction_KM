...
Core scripts are in `scripts/`:
- `scripts/export_db_params.py`: exports building parameter skeletons from PostGIS (1,064 buildings)
...
Working data:
- `params/` — building parameter JSON files (1,064+ from DB export)
- `params/_site_coordinates.json` — georeferenced placement lookup
- `batches/` — photo analysis batch files for agents
- `PHOTOS KENSINGTON/` — 1,867 geotagged field photos + address index
- `gis_scene.py` + `gis_scene.json` — Blender site model
- `outputs/` — rendered Blender output
- `reference_photos/` — reference images
- `legacy_analysis/` — archived old Ollama-based scripts
...
1. Running scripts on a narrow sample first (e.g., `--address "22 Lippincott St"`)
2. Checking enrichment pipeline output: `python scripts/translate_agent_params.py && python scripts/enrich_skeletons.py`
3. Regenerating a known address and comparing against reference photos
